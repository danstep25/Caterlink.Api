...
try {
  if (!empty($input["request"])) {
    ...
    $validationQuery = "SELECT * FROM `unitofmeasurement` WHERE 
      `abbr` = '$abbr' AND
      `description` = '$description' AND
      `isActive`
      ";

    (new Validation($conn, $validationQuery))->isValid(MODULE::UnitOfMeasurement,METHOD::CREATE);
    ...
    $sql = "INSERT INTO `unitofmeasurement` 
      (`abbr`, `description`) 
